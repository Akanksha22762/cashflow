            ...
            <div className="p-6">
              ...
              {loadingTrendReasoning ? (
                <div className="text-center py-8">
                  <div className="text-lg">🤖 AI is analyzing trends...</div>
...
function TrendsClient() {
  ...
  const toggle = (t: string) => {
    setSelected((prev: string[]) => (prev.includes(t) ? prev.filter((x: string) => x !== t) : [...prev, t]));
  };
  ...
  const run = async () => {
    try {
      ...
      const v = await fetch(`${API_BASE}/validate-trend-selection`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ selected_trends: selected })
      });
      ...
      const r = await fetch(`${API_BASE}/run-dynamic-trends-analysis`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ analysis_type: selected })
      });
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <button className="px-4 py-2 bg-blue-600 text-white rounded-md text-sm hover:bg-blue-700 disabled:bg-gray-400" onClick={run} disabled={loading || selected.length === 0}>
          {loading ? "Running…" : `Run Analysis ${selected.length > 0 ? `(${selected.length} options)` : ''}`}
        </button>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="p-3 bg-blue-50 border-2 border-blue-200 rounded-lg">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <span className="text-sm font-bold text-blue-800">
              📊 All Options ({types.length} available)
            </span>
            <span className="text-xs text-blue-600">
              {selected.length === types.length ? 'All selected' : `${selected.length}/${types.length} selected`}
            </span>
          </label>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
          {types.map((t) => (
            <label key={t} className="flex items-center gap-2 p-2 border rounded-md hover:bg-gray-50 cursor-pointer">
              <input type="checkbox" checked={selected.includes(t)} onChange={() => toggle(t)} />
              <span className="text-sm font-medium">{formatTrendName(t)}</span>
            </label>
          ))}
