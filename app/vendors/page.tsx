  ...
  useEffect(() => {
    const fetchVendors = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/get-dropdown-data`, { cache: "no-store" });
    ...
    try {
      ...
      try {
        ...
        if (txRes.ok) {
          ...
          const inflows = txs.filter((t:any) => Number(t.amount) > 0).reduce((s:number, t:any) => s + Number(t.amount || 0), 0);
          const outflows = txs.filter((t:any) => Number(t.amount) < 0).reduce((s:number, t:any) => s + Math.abs(Number(t.amount || 0)), 0);
    ...
    <DashboardLayout>
      <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-gray-500">Uses OpenAI to assign vendors from descriptions.</span>
        </div>
              ...
              <div className="mt-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  <div className="p-6 border rounded-lg bg-white shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div className="text-2xl font-bold text-green-600">${Math.abs(txSummary.inflows).toLocaleString()}</div>
                  </div>
                  <div className="p-6 border rounded-lg bg-white shadow-sm hover:shadow-md transition-shadow">
                    <div className="text-sm text-gray-600 mb-2">Outflows</div>
                    <div className="text-2xl font-bold text-red-600">${Math.abs(txSummary.outflows).toLocaleString()}</div>
                  </div>
                  <div className="p-6 border rounded-lg bg-white shadow-sm hover:shadow-md transition-shadow">
                    <div className="text-sm text-gray-600 mb-2">NetFlow</div>
                    <div className={`text-2xl font-bold ${txSummary.net >= 0 ? 'text-green-600' : 'text-red-600'}`}>${Math.abs(txSummary.net).toLocaleString()}</div>
                  </div>
                  <div className="p-6 border rounded-lg bg-white shadow-sm hover:shadow-md transition-shadow">
                        ...
                        {vendorTxs.map((t:any, i:number) => {
                          ...
                          return (
                            <tr key={i} className="border-t">
                              ...
                              <td className={`py-2 pr-3 whitespace-nowrap ${Number(t.amount) >= 0 ? 'text-green-600' : 'text-red-600'}`}>${Math.abs(Number(t.amount || 0)).toLocaleString()}</td>
                              <td className="py-2 pr-3 whitespace-nowrap">{t.type || ''}</td>
                              <td className="py-2 pr-3 whitespace-nowrap">{t.category || 'Uncategorized'}</td>
                            </tr>
                          );
                        })}
                      </tbody>
            ...
            <div className="bg-white rounded-lg max-w-4xl w-full max-h-[80vh] overflow-y-auto">
              <div className="p-6">
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-xl font-semibold">🤖 AI Vendor Analysis: {selectedVendor}</h2>
                  <button 
                ...
                {loadingVendorReasoning ? (
                  <div className="text-center py-8">
                    <div className="text-lg">🤖 AI is analyzing vendor...</div>
