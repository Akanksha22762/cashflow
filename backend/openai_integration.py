...
Enhanced description (one line only):"""
    ...
    def categorize_transactions(self, descriptions: List[str], model: str = None) -> List[str]:
        ...
        batch_prompt = f"""You are an expert accountant who classifies cash flows for ANY industry. For every transaction below, output the proper cash-flow-statement category using standard GAAP/IFRS definitions.

TRANSACTIONS:
{transactions_text}

CATEGORY DEFINITIONS (always choose exactly ONE per line):

1. OPERATING ACTIVITIES → Core revenue/expense cash flows that arise from delivering products or services and running the business day to day.
   Examples: customer receipts, service revenue, product sales, inventory and raw-material purchases, vendor payments for supplies, payroll, benefits, taxes, utilities, rent, insurance, routine maintenance, professional services, transportation or logistics fees, customer refunds, routine settlements with customers or suppliers.

2. INVESTING ACTIVITIES → Cash flows that acquire or dispose of long-lived assets or financial investments.
   Examples: capital expenditures, construction costs, equipment or software purchases meant to be capitalized, proceeds from selling PP&E, acquisitions or divestitures, purchase or sale of debt/equity securities that are not cash equivalents, loans made to third parties and their collections.

3. FINANCING ACTIVITIES → Cash flows that change the entity’s capital structure or debt obligations.
   Examples: new borrowings, loan drawdowns, bond issuances, principal repayments, interest payments, lease liabilities, equity infusions, share buybacks, dividends, capital contributions or distributions between parent and subsidiary.

INSTRUCTIONS:
- Think generically: do NOT rely on industry-specific shortcuts.
- Treat any description that explicitly mentions “capital expenditure”, “capitalized”, “PPE”, “asset purchase”, “equipment upgrade”, or similar as Investing unless evidence shows it is routine maintenance.
- Treat any loan proceeds, repayments, interest, or equity movements as Financing.
- Treat routine operating expenses, consumables, payroll, cost of goods sold, and customer settlements as Operating.
- Never default all records to Operating; analyze each description carefully even if several are similar.
...
Answer with category per line only:"""
        ...
        for line in lines:
            ...
            index_str, category_text = match.groups()
