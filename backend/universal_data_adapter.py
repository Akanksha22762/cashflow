...
class UniversalDataAdapter:
    ...
    def load_and_adapt(cls, file_path: str, test_limit: int = None) -> pd.DataFrame:
        ...
        original_count = len(adapted_data)
        
        if test_limit and original_count > test_limit:
            logger.info(f"🧪 TEST MODE: Limiting dataset from {original_count} to {test_limit} transactions")
            adapted_data = adapted_data.head(test_limit)
            logger.info(f"✅ Dataset limited to first {test_limit} transactions")
